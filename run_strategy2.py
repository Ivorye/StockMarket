...
cursor.execute('''SELECT d.symbol, s.st_code, d.pct_chg,
                         CASE WHEN p.vol > 0 THEN ROUND(d.vol / p.vol, 2) ELSE 0 END,
...
result = [dict(symbol=row[0], st_code=row[1], pct_chg=row[2],
               vol_ratio=row[3], gap=row[4]) for row in cursor.fetchall()]
