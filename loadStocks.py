            ...
            if result==0:
                try:
                    ...
                    if i==ln-1:
                        db.commit()
                        commited = True
                        print(ln, "All records have been loaded to database")
                except:
                    print("exception throw")
                    db.rollback()
            if needcommit == True and commited == False:
                db.commit()
...
#输入：DF数据帧，开始日期和结束日期同一天。批量调用pro.daily
def batchInsertNewTransactonRecordForAllStocks(df='',transaction_date='',db=''):
    lst1 = df.ts_code,df.symbol
    process = True
    pos = 0
    while process:
        batchInsert(lst1,pos,transaction_date,db)
        pos = pos + 500
        if len(lst1) < 500:
            process = False
    db.close()

def batchInsert(lst1='',pos='', transaction_date='',db=''):
    cursor = db.cursor()
    pro = ts.pro_api("4d47c02a8bb025881c9dd9e3c36d25139ab5b429a73353e566fc02a9")
    data = pro.daily(ts_code=','.join(lst1[0][pos:500]),start_date=transaction_date,end_date=transaction_date)
    l=len(data)
    for i in range(0,l):
        table="gp%s"%lst1[1][i]
        sql0="select trade_date from %s where trade_date =%s" %(table,data.loc[i].trade_date)
        sql="insert into %s(trade_date,openp,high,low,closep,preclose,changes,pct_chg,vol,amount) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" %(table,data.loc[i].trade_date,data.loc[i].open,data.loc[i].high,data.loc[i].low,data.loc[i].close,data.loc[i].pre_close,data.loc[i].change,data.loc[i].pct_chg,data.loc[i].vol,data.loc[i].amount)
        result=cursor.execute(sql0)
        print(i, " records have been processed")
        if result==0:
            try:
                cursor.execute(sql)
            except:
...
def getJDZF(df='',start_date='',end_date='',rate='',db=''):
    ...
    for i in range(0,l):
        ...
        try:
            rst1 = cursor.execute(sql)
            if rst1 !=0 :
                p1 = cursor.fetchall()
                rst2 = cursor.execute(sql2)
                if rst2 !=0 :
                    p2 = cursor.fetchall()
                    p3 = p1[0][0]
                    p4 = p2[0][0]
